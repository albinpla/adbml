...
try:
    ...
    number = 10
print(number)
classif = linear_model.SGDClassifier()
...
for content in urls:
    ...
    fullset.append(list(url_class.values()))
    resSet.append(res);
# print(fullset[0])

training_set = fullset
print("Training SGDC")
classifier = classif.fit(training_set,resSet)
...
print(" saved to ./classifiers/sgdc%s.pickle"%number)
save_classifier = open("./classifiers/sgdc%s.pickle"%number,"wb")
